...
const Navbar = () => {
    ...
    // scrll effect to change navbar color
    useEffect(() => {
        // Set initial scroll state based on pathname
        const shouldScroll = location.pathname !== "/";
        setIsScrolled(shouldScroll || window.scrollY > 10);
    ...
    return (
            ...
            xl:px-32 transition-all duration-500 z-50 
            ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    <button className="absolute top-4 right-4" onClick={() => setIsMenuOpen(false)}>
                       <img src={assets.closeMenu} alt="close" className="h-6.5" />
                    </button>
